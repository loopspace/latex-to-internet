...
{
  \seq_set_split:Nnn \l__xhtm_args_seq {,} {{#1}}
  \xhtm_use_hook:n {pre push tag}
...
{
  \seq_set_split:Nnn \l__xhtm_args_seq {,} {{#1},{#2}}
  \xhtm_use_hook:n {pre open tag}
  \xhtm_mbox:n {}
  \xhtm_push_tag:n {#1}
  <#1
  \tl_if_empty:nF{#2}
...
\cs_generate_variant:Nn \xhtm_open_tag:nn {x,V,xx,nx}

\cs_new_protected_nopar:Npn \xhtm_close_tag:n #1
{
  \seq_set_split:Nnn \l__xhtm_args_seq {,} {{#1}}
  \xhtm_use_hook:n {pre close tag}
  \xhtm_pop_tag:n {#1}
  </#1>
}
...
\cs_new_nopar:Npn \xhtm_section_header:nnn #1#2#3
{
  ...
  \seq_set_split:Nnn \l__xhtm_args_seq {,} {{#1},{#2},{#3}}
  \xhtm_use_hook:n {post section header}
}
...
\NewDocumentEnvironment {xhtm description} {m}
{
  \xhtm_close_par:
  \xhtm_open_tag:nn {dl} {}
  \xhtm_set_hook:nn {after list item} {\xhtm_close_tag:n {dd}}
...
\cs_new_nopar:Npn \xhtm_main_item:w [#1]
{
  \xhtm_close_par:
  {
    \xhtm_use_hook:n {after list item}
  }
...
{
  \seq_set_split:Nnn \l__xhtm_args_seq {,} {{#1}}
  \xhtm_use_hook:n {pre bibliography header}
  \section*{References}
  \seq_set_split:Nnn \l__xhtm_args_seq {,} {{#1}}
  \xhtm_use_hook:n {post bibliography header}
