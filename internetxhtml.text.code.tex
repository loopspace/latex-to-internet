...
\tl_new:N \l__xhtm_anchor_tmp_tl
\tl_new:N \l__xhtm_link_tl
...
\tl_new:N \l__xhtm_baseurl_tl
 ...
 \cs_new_nopar:Npn \xhtm_set_anchor_type:n #1
 {
   \tl_set:Nn \l__xhtm_anchor_tl {#1}
 }

 \DeclareDocumentCommand \SetAnchorType { m }
 {
   \xhtm_set_anchor_type:n {#1}
 }
 
 \cs_new_nopar:Npn \xhtm_set_link_type:n #1
 {
   \tl_set:Nn \l__xhtm_link_tl {#1}
 }

 \DeclareDocumentCommand \SetLinkType { m }
 {
   \xhtm_set_link_type:n {#1}
 }
...
\cs_new_nopar:Npn \xhtm_num_section:nnn #1#2#3
{
  \xhtm_close_par:
  \txt_newline:
  \tl_set_eq:NN \l__xhtml_anchor_tmp_tl \l__xhtml_anchor_tl
  \tl_set:Nn \l__xhtm_anchor_tl {on next}
  \refstepcounter{#1}
  \tl_set_eq:NN \l__xhtml_anchor_tl \l__xhtml_anchor_tmp_tl
...
\seq_new:N \l__xhtm_par_classes_seq
...
\cs_new_nopar:Npn \xhtm_reopen_par:
{
  \xhtm_if_current_tag_is_par:T
  {
    \xhtm_close_tag:n {p}
    \xhtm_linebreak:
    \tl_clear:N \l__xhtm_tmpa_tl
    \seq_if_empty:NF  \l__xhtm_par_classes_seq
    {
      \tl_set:Nx \l__xhtm_tmpa_tl {class="\seq_use:Nn \l__xhtm_par_classes_seq {\c_space_token}"}
    }
    \xhtm_open_tag:nn {p}{\tl_use:N \l__xhtm_tmpa_tl}
  }
}
...
}
{
  ...
  {
    \seq_push:Nn \l__xhtm_par_classes_seq {#1}
  }
}
...
\cs_new_nopar:Npn \xhtm_enumerate:n #1
{
  \xhtm_close_par:
...
\cs_new_nopar:Npn \xhtm_end_enumerate:
{
  \xhtm_close_par:
  \xhtm_close_tag:n {li}
  \xhtm_close_tag:n {ol}
...
\cs_new:Npn \xhtm_replace_verb_chars_a:
{
  \tl_replace_all:NnV \l__xhtm_tmpa_tl {&} \xhtm_amp:
  \tl_replace_all:NnV \l__xhtm_tmpa_tl {<} \xhtm_lt:
  \tl_replace_all:NnV \l__xhtm_tmpa_tl {>} \xhtm_gt:
  \tl_replace_all:NnV \l__xhtm_tmpa_tl {$} \xhtm_dollar:%$
}
...
\char_set_catcode_other:N &
\char_set_catcode_other:N $%$
\cs_new:Npn \xhtm_replace_verb_chars_o:
{
  \tl_replace_all:NnV \l__xhtm_tmpa_tl {&} \xhtm_amp:
  \tl_replace_all:NnV \l__xhtm_tmpa_tl {<} \xhtm_lt:
  \tl_replace_all:NnV \l__xhtm_tmpa_tl {>} \xhtm_gt:
...
\cs_new:Npn \xhtm_replace_verb_chars:Nn #1#2
{
  \tl_set:Nn \l__xhtm_tmpa_tl {#2}
  \xhtm_replace_verb_chars_a:
  \xhtm_replace_verb_chars_o:
  \tl_set_eq:NN #1 \l__xhtm_tmpa_tl
}

\cs_generate_variant:Nn \xhtm_replace_verb_chars:Nn {NV}

\cs_new:Npn \xhtm_verb:n #1
{
  \xhtm_replace_verb_chars:Nn \l__xhtm_tmpa_tl {#1}
  \xhtm_open_tag:nn {code}{}
  \tl_use:N \l__xhtm_tmpa_tl
  \xhtm_close_tag:n {code}
}

\cs_generate_variant:Nn \xhtm_verb:n {V,N}
...
\tl_new:N \l__xhtm_code_tl

\cs_set:Npn \verbatim
{
  \xhtm_open_code_block:
  \group_begin:
...
\cs_set:Npn \endverbatim
{
  ...
  \group_end:
  \xhtm_close_code_block:
}

\cs_set:Npn \xhtm_open_code_block:
{
  \xhtm_close_par:
  \txt_use_hook:nn {xhtm} {pre verbatim}
  \xhtm_open_tag:nn {pre} {}
  \xhtm_open_tag:nV {code} \l__xhtm_code_tl
}

\cs_set:Npn \xhtm_close_code_block:
{
  \xhtm_close_tag:n {code}
  \xhtm_close_tag:n {pre}
  \txt_use_hook:nn {xhtm} {post verbatim}
  \xhtm_open_par:
}
...
\cs_set:Npn \xhtm_make_verbatim:n #1
{
  \tl_put_right:Nn \l_tmpa_tl {\cs_new:cpn {xhtm verbatim #1} ##1}
...
\DeclareDocumentCommand \newverbatim {m o o} {
  \xhtm_make_verbatim:n {#1}
  \IfNoValueTF {#2}
  {
    \cs_new:cpn {#1}
    {
      \cs_set_eq:Nc \@xverbatim {xhtm verbatim #1}
      \verbatim
    }
  }
  {
...
\clist_new:N \l__xhtm_grext_clist
\clist_set:Nn \l__xhtm_grext_clist {png,PNG,jpg,JPG,jpeg,JPEG,gif,GIF}
...
\DeclareDocumentCommand \xhtm_includegraphics {o m}
{
  ...
  \file_if_exist:nF {#2}
  {
    \clist_map_inline:Nn \l__xhtm_grext_clist
    {
  ...
  \tl_set:Nx \l_tmpb_tl
  {
    \exp_not:N \xhtm_pre_graphics:n {\l_tmpa_tl}
    \exp_not:N \xhtm_empty_tag:nn {img} {src="\l_tmpa_tl"~ class="includegraphics"~ alt=""}
    \exp_not:N \xhtm_post_graphics:n {\l_tmpa_tl}
  }
  \tl_use:N \l_tmpb_tl
}
...
\NewDocumentCommand \tableofcontents {}
{
  ...
  {
    \prg_replicate:nn {\l__xhtm_toclvl_int} {\xhtm_end_enumerate:}
  }
  \xhtm_close_par:
  \xhtm_close_tag:n {div}
  \xhtm_open_par:
...
\cs_new_nopar:Npn \xhtm_tocline:nnn #1#2#3
{
  ...
  {
    \prg_replicate:nn {\l__xhtm_toclvl_int - #1} {\xhtm_end_enumerate:}
  }
  \int_compare:nT {\l__xhtm_toclvl_int < #1}
  {
    \prg_replicate:nn {#1 - \l__xhtm_toclvl_int} {\xhtm_enumerate:n{}}
  }
  \item #2
...
\NewDocumentCommand \loadcss {O {all} m}
{
  ...
  {
    \xhtm_linebreak:
    \xhtm_empty_tag:nn {link} {href="#2.css"~ media="#1"~ rel="stylesheet"~ type="text/css"}
  }
}
...
\NewDocumentCommand \loadfullcss {O {all} m}
{
  ...
  {
    \xhtm_linebreak:
    \xhtm_empty_tag:nn {link} {href="#2"~ media="#1"~ rel="stylesheet"~ type="text/css"}
  }
}
...
\NewDocumentCommand \loadfulljs {v}
{
  ...
  \tl_gput_right:Nn \l__xhtm_jsfiles_tl
  {
    \xhtm_empty_tag:nn {script} {src="#1"~ type="text/javascript"}
  }
}

\NewDocumentCommand \addheadtag {m v}
{
  \tl_gput_right:Nn \l__xhtm_headtags_tl
  {
    \xhtm_linebreak:
    \xhtm_empty_tag:nn {#1} {#2}
  }
}
...
\tl_new:N \l__xhtm_licence_tl

\prop_new:N \l__xhtm_cclicences_prop
\prop_put:Nnn \l__xhtm_cclicences_prop {by} {Attribution}
\prop_put:Nnn \l__xhtm_cclicences_prop {nc} {NonCommercial}
\prop_put:Nnn \l__xhtm_cclicences_prop {nd} {NoDerivatives}
\prop_put:Nnn \l__xhtm_cclicences_prop {sa} {ShareAlike}

\NewDocumentCommand \cclicence { m }
{
  \tl_to_lowercase:n {\tl_set:Nn \l_tmpa_tl {#1}}
  \seq_set_split:NnV \l_tmpa_seq {-} \l_tmpa_tl
  \seq_clear:N \l_tmpb_seq
  \seq_map_inline:Nn \l_tmpa_seq
  {
    \prop_get:NnNT \l__xhtm_cclicences_prop {##1} \l_tmpb_tl
    {
      \seq_put_right:NV \l_tmpb_seq \l_tmpb_tl
    }
  }
  \tl_set:Nx \l__xhtm_licence_tl
  {
  <a~ rel="license"~ href="http://creativecommons.org/licenses/\tl_use:N \l_tmpa_tl/4.0/"><img~ alt="Creative~ Commons~ License"~ style="border-width:0"~ src="http://i.creativecommons.org/l/\tl_use:N \l_tmpa_tl/4.0/88x31.png"~ /></a>
%This~ work~ is~ licensed~ under~ a~ <a~ rel="license"~ href="http://creativecommons.org/licenses/\tl_use:N \l_tmpa_tl/4.0/">Creative~ Commons~ \seq_use:Nn \l_tmpb_seq {-}~ 4.0~ International~ License</a>.
  }
}
...
\xhtm_at_begindocument:n
{
  ...
  \xhtm_open_tag:nn {div} {id="background"}
  \xhtm_close_tag:n {div}
  \xhtm_open_tag:nn {div} {id="content"}
  \xhtm_open_par:
...
\NewDocumentEnvironment {thebibliography} {m}
{
  ...
  \int_set:Nn \l__xhtm_bibitem_int {2*(2 + \tl_count:V \l_tmpa_tl)/3+1}
  \xhtm_enumerate:n {class="bibliography"~ style="padding-left:~\int_use:N \l__xhtm_bibitem_int em"}
  \cs_set_eq:NN \xhtm_list_label:n \xhtm_bibitem_label:n
}
{
  \xhtm_end_enumerate:
}
...
\cs_new:Npn \xhtm_bibitem_label:n #1
{
  \xhtm_open_par:
  \xhtm_open_tag:nn {span} {class="listlabel"~ style="margin-left:~\int_eval:n {-\l__xhtm_bibitem_int - 1} em;~ width:~\int_use:N \l__xhtm_bibitem_int em;"}
  #1
  \xhtm_close_tag:n {span}
}
...
\cs_set:Npn \maketitle {
  \xhtm_close_par:
  \xhtm_open_tag:nn {div} {id="titlepage"}
  \tl_if_eq:NNF \@title \l__xhtm_title_tl
  {
    \xhtm_linebreak:
    \xhtm_open_tag:nn {div}{id="title"}
    \xhtm_open_par:
    \@title
    \xhtm_close_par:
    \xhtm_close_tag:n {div}
  ...
  {
    \xhtm_linebreak:
    \xhtm_open_tag:nn {div}{id="author"}
    \xhtm_open_par:
    \@author
    \xhtm_close_par:
    \xhtm_close_tag:n {div}
  ...
  \xhtm_linebreak:
  \xhtm_open_tag:nn {div}{id="date"}
  \xhtm_open_par:
  \@date
  \xhtm_close_par:
  \xhtm_close_tag:n {div}
  \tl_if_empty:NF \l__xhtm_licence_tl
  {
    \xhtm_open_tag:nn {div}{id="licence"}
    \xhtm_open_par:
    \tl_use:N \l__xhtm_licence_tl
    \xhtm_close_par:
    \xhtm_close_tag:n {div}
  }
  \xhtm_close_tag:n {div}
...
\tl_new:N \l__xhtm_today_tl
\tl_set:Nx \l__xhtm_today_tl {
  \the\year-
  \two@digits{\the\month}-
  \two@digits{\the\day}
}

\tl_set_eq:NN \today \l__xhtm_today_tl
...
% Ooops!  Marginpar ought to be a command, not an environment.

\int_new:N \l__xhtm_marginpar_int

\RenewDocumentEnvironment {marginpar} {}
{
  \int_gincr:N \l__xhtm_marginpar_int
  \xhtm_open_tag:nx {a}
  {
    class="marginpar_src"~
    id="marginpar_src.\int_use:N \l__xhtm_marginpar_int"~
    href="\#marginpar_tgt.\int_use:N \l__xhtm_marginpar_int"
  }
  \int_use:N \l__xhtm_marginpar_int
  \xhtm_close_tag:n {a}
  \xhtm_open_tag:nn {div} {class="marginpar"}
  \xhtm_open_tag:nn {div} {}
  \xhtm_open_par:
  \xhtm_open_tag:nn {a} {
      class="marginpar_tgt"~
      id="marginpar_tgt.\int_use:N \l__xhtm_marginpar_int"~
      href="\#marginpar_src.\int_use:N \l__xhtm_marginpar_int"
    }
  \int_use:N \l__xhtm_marginpar_int
  \xhtm_close_tag:n {a}
}
...
\tl_new:N \l__xhtm_capend_tl
\tl_set:Nn \l__xhtm_capend_tl {
  \xhtm_close_tag:n {p}
  \xhtm_open_par:
}

\msg_new:nnnn {XHTML} {caption~ outside~ float} {\exp_not:N \caption~ outside~ float~ at~ line~ \msg_line_context:} {}

\DeclareDocumentCommand \caption {O{}} {
  \tl_if_exist:NTF \@captype
  {
    \refstepcounter\@captype
    \addcontentsline{\use:c{ext@\@captype}}{\@captype}%
    {\protect\numberline{\use:c{the\@captype}}{\ignorespaces #1}}%
  }
  {
    \msg_warning:nnnn {XHTML} {caption~ outside~ float} {} {}
  }
  \xhtm_close_par:
  \xhtm_open_tag:nn {p} {class="caption"}
  \use:c{fnum@\@captype}:~
  \peek_catcode_remove:NT \c_group_begin_token
  {
    \message{Doing~caption~group}
    \c_group_begin_token
    \group_insert_after:N \l__xhtm_capend_tl
  }
}
...
\cs_set_eq:NN \xhtm_index_ref:n \ref
\cs_generate_variant:Nn \xhtm_index_ref:n {V}

\AtBeginDocument{
  \@ifpackageloaded{cleveref}{%
    \cs_gset_eq:NN \xhtm_index_ref:n \cref
  }{}
}
