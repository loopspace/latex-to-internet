...
\cs_new_nopar:Npn \itex_text_pushtag:
{
  \tl_set:Nx \l_tmpa_tl {
    ...
    {\seq_item:Nn \l__xhtm_args_seq {2}}
  }
...
\cs_new_nopar:Npn \itex_text_group:
{
  ...
  \xhtm_set_hook:nn {pre open tag}
  {
    \itex_text_pushtag:
  }
  \xhtm_set_hook:nn {pre close tag}
  {
    \itex_text_poptag:
  }
  \itextext
  \{
%  \xhtm_open_tag:nn {semantics} {}
%  \xhtm_open_tag:nn {annotation_xml} {encoding='XHTML1.1'}
    \tfont
...
\cs_set:Npn \mathbin #1 {#1}
\cs_set:Npn \mathrel #1 {#1}
