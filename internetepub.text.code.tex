...
\xhtm_at_enddocument:n
{
  ...
  \int_step_inline:nnnn {1} {1} {\l__epub_bibpage_int}
  {
...
\cs_new_nopar:Npn \epub_split_section:n #1
{
  ...
  {
    \seq_item:Nn \l__xhtm_args_seq {1} + \l__xhtm_sec_int
  }
  \bool_if:nT {
    \int_compare_p:n {
    ...
    &&
    \int_compare_p:n {
      \l__epub_curr_section_int >= \int_use:c {l__epub_#1_level_int}
    }
  }
  {
    \xhtm_close_par:
...
\cs_new:Npn \epub_phantom_section:
{
  ...
  \seq_set_split:NnV \l__xhtm_args_seq {,} \l_tmpa_tl
\xhtm_use_hook:n {pre section header}
  % This ensures we don't break after a \phantomsection
  \int_gset:Nn \l__epub_curr_section_int {0}
...
\xhtm_set_hook:nn {link}
{
  \tl_if_exist:cT {epub_page_  \seq_item:Nn \l__xhtm_args_seq {2}}
  {
    \tl_set:Nx \l__xhtm_baseurl_tl {\jobname \tl_use:c {epub_page_  \seq_item:Nn \l__xhtm_args_seq {2}} .xhtml}
  }
}
...
\xhtm_set_hook:nn {post bibliography header}
{
  \write\@auxout{\string\epubbibpage {\use:c {the epub segments}}}  
}
