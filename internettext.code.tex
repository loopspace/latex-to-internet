...
\DeclareDocumentCommand \newtextcommand { s o m O{0} o }
{
  ...
  \IfNoValueTF{#5}
  {
    \int_compare:nTF {#4 = 0}
    {
      \cs_set_protected:Npx #3 {\tl_use:N \l_tmpa_tl \tl_use:N \l_tmpb_tl}
    }
    {
      \cs_set_protected:Npx #3 {\tl_use:N \l_tmpa_tl \exp_not:N \txt_gobble_mopt:nn {#4} }
    }
  }
  {
    \cs_set_protected:Npx #3 {\tl_use:N \l_tmpa_tl \exp_not:N \txt_gobble_oopt:nn {#4} {#5} }
  }
}
...
\DeclareDocumentCommand \newtextenvironment { m O{0} o }
{
  \IfNoValueTF{#3}
  {
    \int_compare:nTF {#2 = 0}
    {
      \cs_set:cpn {#1} {\string\begin\txt_lbrace:#1\txt_rbrace:}
    }
    {
      \cs_set:cpn {#1} {\string\begin\txt_lbrace:#1\txt_rbrace: \txt_gobble_mopt:nn {#2} }
    }
  }
  {
    \cs_set:cpn {#1} {\string\begin\txt_lbrace:#1\txt_rbrace: \txt_gobble_oopt:nn {#2} {#3} }
  }
  \cs_set:cpn {end #1} {\string\end\txt_lbrace:#1\txt_rbrace:}
}
...
\cs_generate_variant:Nn \txt_only_chars:nn {Vn,nV,VV}
