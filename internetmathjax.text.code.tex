...
% Originates from the itex module 

\tl_new:N \l__mjax_math_open_tl
\tl_new:N \l__mjax_math_close_tl
\tl_new:N \l__mjax_dmath_open_tl
\tl_new:N \l__mjax_dmath_close_tl


% Define the options for the mjax module
\keys_define:nn {mjax options}
{
  delimiter .choice:,
  delimiter / dollar .code:n = {
    \tl_set:Nn \l__mjax_math_open_tl {\$}
    \tl_set:Nn \l__mjax_math_close_tl {\$}
    \tl_set:Nn \l__mjax_dmath_open_tl {\$\$}
    \tl_set:Nn \l__mjax_dmath_close_tl {\$\$}
  },
  delimiter / parenthesis .code:n = {
    \tl_set:Nn \l__mjax_math_open_tl {\@backslashchar(}
    \tl_set:Nn \l__mjax_math_close_tl {\@backslashchar)}
    \tl_set:Nn \l__mjax_dmath_open_tl {\@backslashchar[}
    \tl_set:Nn \l__mjax_dmath_close_tl {\@backslashchar]}
  },
  delimiter / m .code:n = {
    \tl_set:Nn \l__mjax_math_open_tl {[m]}
    \tl_set:Nn \l__mjax_math_close_tl {[/m]}
    \tl_set:Nn \l__mjax_dmath_open_tl {[m]}
    \tl_set:Nn \l__mjax_dmath_close_tl {[/m]}
  },
}

\keys_set:nn {mjax options} {delimiter = parenthesis}

% Get and set the options for the markdown-extra module
\prop_get:NnNT  \l__txt_popts_prop {mjax} \l_tmpa_tl
{
  \keys_set:nV {mjax options} \l_tmpa_tl
}


\cs_new:Npn \mjax_activate_dollar:
{
  \char_set_catcode_active:N \$%$
}
\mjax_activate_dollar:

\AtBeginDocument{
  \mjax_activate_dollar:
}

\cs_new_nopar:Npn \mjax_superscript:n #1
{
  \tl_use:N \l__txt_caret_tl \txt_lbrace:
    {
      \bool_if:NTF \l__mjax_script_bool
      {
        \bool_set_true:N \l__mjax_scriptscript_bool
      }
      {
        \bool_set_true:N \l__mjax_script_bool
      }
      #1
    }
    \txt_rbrace:
...
\cs_new_nopar:Npn \mjax_subscript:n #1
{
...
\cs_new_nopar:Npn \mjax_amp: {&}
\group_end:


\bool_new:N \l__mjax_display_bool
\bool_new:N \l__mjax_maths_bool
\bool_new:N \l__mjax_numbered_bool
\bool_new:N \l__mjax_text_bool
\bool_new:N \l__mjax_script_bool
\bool_new:N \l__mjax_scriptscript_bool

\cs_new_nopar:Npn \mjax_maths_start:
{
...
\NewDocumentCommand \mjaxSetAllCatcodes {}
{
...
\cs_if_exist:NT \epub_math_in_toc:
{
  ...
  \cs_set:Npn \epub_math_in_toc:
  {
    \mjax_math_in_toc:
    \AtBeginDocument{
      \@writefile{toc}{\mjaxSetAllCatcodes}
    }
  }
}
...
\cs_new_protected_nopar:Npn \mjax_dollar:
{
...
\cs_new:Npn \mjax_double_dollar:w $%$
{
  \bool_if:NT \l__mjax_maths_bool
  {
    \bool_set_true:N \l__mjax_display_bool
    \mjax_maths_start:
  }
}

\cs_new:Npn \mjax_single_dollar:
{
  \bool_if:NT \l__mjax_maths_bool
  {
    \mjax_maths_start:
...
\cs_set_eq:NN $ \mjax_dollar: %$

\NewDocumentCommand \mjaxmathscommand {m o o}
{
  \cs_if_exist:NT #1
  {
    \cs_set_eq:cN {mjax_text_\cs_to_str:N #1:} #1
  }
  \IfNoValueTF {#2}
  {
    \newtextcommand #1
  ...
  \cs_set_eq:cN {mjax_maths_\cs_to_str:N #1:} #1
  \cs_set_nopar:Npn #1
  {
    \bool_if:NTF \l__mjax_maths_bool
    {
      \use:c {mjax_maths_\cs_to_str:N #1:}
    }
    {
      \use:c {mjax_text_\cs_to_str:N #1:}
    }
  }
}

\cs_new_nopar:Npn \mjax_label:n #1
{
  \txt_only_chars:nn {alpha} {#1}
  \string\label \txt_lbrace:\tl_use:N \l__txt_chars_tl \txt_rbrace:
}

\NewDocumentEnvironment {mjax equation} {}
{
  \bool_set_true:N \l__mjax_numbered_bool
  \[
}
{
  \]
...
\cs_new_nopar:Npn \mjax_restore_equation:
{
  \cs_set_eq:Nc \equation {mjax equation}
  \cs_set_eq:Nc \endequation {end mjax equation}
}

% hyperref redefines equation
\AtBeginDocument{
  \mjax_restore_equation:
}
...
\cs_new_nopar:Npn \mjax_ensuremaths:
{
  \bool_if:NTF \l__mjax_maths_bool
  {
    \use:n
  ...
  {
    $ \mjax_ensuremaths_end:n %$
  }
}

\cs_new_nopar:Npn \mjax_ensuremaths_end:n #1
{
  #1$%$
}
...
{
  \bool_set_false:N \l__mjax_numbered_bool
  \txt_use_hook:nn {mjax} {begin align}
  $$
  \begin{aligned}
  \txt_newline:
}
...
{
  \bool_set_false:N \l__mjax_numbered_bool
  \txt_use_hook:nn {mjax} {begin gather}
  $$
  \begin{gathered}
  \txt_newline:
}
{
  ...
  \end{gathered}
  $$
  \txt_use_hook:nn {mjax} {end gather}
}
...
{
  \bool_set_false:N \l__mjax_numbered_bool
  \txt_use_hook:nn {mjax} {begin multline}
  $$
  \begin{aligned}
  \txt_newline:
}
{
  ...
  \end{aligned}
  $$
  \txt_use_hook:nn {mjax} {end multline}
}
...
{
  \bool_set_false:N \l__mjax_numbered_bool
  \txt_use_hook:nn {mjax} {begin equation}
  $$
}
{
  $$
  \txt_use_hook:nn {mjax} {end equation}
}
...
\NewDocumentCommand \DeclaremjaxOperator {m}
{
  \newtextcommand #1
}
...
\RenewDocumentCommand \not {}
{
  \peek_after:Nw \mjax_if_not:
}

\cs_new:Npn \mjax_if_not:
{
...
\tl_new:N \l__mjax_endtext_tl
\tl_set:Nn \l__mjax_endtext_tl {
\bool_set_false:N
\l__mjax_text_bool
  \mjax_text_closetags:
  \txt_rbrace:
...
\cs_new_nopar:Npn \mjax_text:n #1
{
  \mjaxtext \mjaxgroup {#1}
}

\seq_new:N \l__mjax_tag_seq

\cs_new_nopar:Npn \mjax_text_pushtag:
{
  \tl_set:Nx \l_tmpa_tl {
    {\seq_item:Nn \l__txt_args_seq {1}}
    {\seq_item:Nn \l__txt_args_seq {2}}
  }
  \seq_gpush:NV \l__mjax_tag_seq \l_tmpa_tl
}

\cs_generate_variant:Nn \msg_warning:nnnn {nnVV}

\cs_new:Npn \mjax_text_poptag:
{
  \seq_gpop:NN \l__mjax_tag_seq \l_tmpa_tl
}

\seq_new:N \l__mjax_texttags_seq
\cs_new:Npn \mjax_text_closetags:
{
  \seq_gset_eq:NN \l__mjax_texttags_seq \l__mjax_tag_seq
  \seq_map_inline:Nn \l__mjax_tag_seq
  {
    \xhtm_close_tag:x {\tl_item:nn {##1} {1}}
  }
  \seq_gclear:N \l__mjax_tag_seq
}
\cs_new:Npn \mjax_text_opentags:
{
  \seq_reverse:N \l__mjax_texttags_seq
  \seq_map_inline:Nn \l__mjax_texttags_seq
  {
    \xhtm_open_tag:xx {\tl_item:nn {##1} {1}} {\tl_item:nn {##1} {2}}
  }
  \seq_gclear:N \l__mjax_texttags_seq
}

\cs_new_nopar:Npn \mjax_text_group:
{
  \bool_set_true:N \l__mjax_text_bool
  \c_group_begin_token
  \cs_set_eq:NN $ \mjax_text_dollar: %$
  \bool_set_false:N \l__mjax_maths_bool
  \group_insert_after:N \l__mjax_endtext_tl
...
\cs_new_nopar:Npn \mjax_text_dollar:
{
  \bool_if:NTF \l__mjax_maths_bool
  {
    \mjaxtext\txt_lbrace:
...
% Here followeth the list of mjax commands,
...
\newtextcommand{\mjaxnum}[1]
\newtextcommand{\mathraisebox}[1]
\newtextcommand{\includegraphics}
\newtextcommand[\text]{\mjaxtext}
\newtextcommand[\array]{\mjaxarray}
\newtextcommand[\align]{\mjaxalign}
\newtextcommand[\space]{\mjaxspace}[3]
\newtextcommand[]{\mjaxgroup}[1]
\newtextcommand[\text]{\textrm}[1]
\newtextcommand[\\]{\mjaxnl}
...
\RenewDocumentCommand \mathchoice {m m m m}
{
  ...
  {
    \bool_if:NTF \l__mjax_scriptscript_bool
    {
      #4
    }
    {
